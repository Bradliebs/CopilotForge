...
EXPECTED OUTPUT:
    [FileManager] Started
    [FileManager] Analyzing 42 files in /Users/Brad/Downloads
    [Copilot] Suggested grouping strategy: by extension
    [DRY RUN] photo1.jpg → images/photo1.jpg
      Reason: Grouped by extension: .jpg
...
class FileOrganizer:
    ...
    Usage:
        ...
        await organizer.organize(target_folder="/path/to/folder", dry_run=True)
    ...
    def _group_by_extension(self, files: list[FileInfo], target_folder: str) -> list[FileOperation]:
        ...
        for file in files:
            subfolder = extension_map.get(file.extension, "other")
...
async def main() -> None:
    ...
    try:
        ...
        await organizer.organize(
            ...
            strategy="extension",
            dry_run=True,
            exclude_patterns=[".*", "*.tmp"],  # Skip hidden files and temp files
        )
