...
| **Build an autonomous dev loop** | The Ralph Loop | `task-loop.ts` / `.py` | 🔴 |
...
| **I want AI to build my project from a plan** | The Ralph Loop + IMPLEMENTATION_PLAN.md | `cookbook/task-loop.ts` | 🔴 |
| **Execute my implementation plan** | Say "run the plan" | The Ralph Loop reads IMPLEMENTATION_PLAN.md, implements tasks one by one | — |
...
18. **`task-loop`** — The Ralph Loop (autonomous plan executor)
    - What: Pick task → implement → validate → commit → repeat
    - When: Building your project from `IMPLEMENTATION_PLAN.md` autonomously
...
| **The Ralph Loop** | A tireless worker that executes your plan while you sleep — picks a task, does it, moves on |
...
- Use **The Ralph Loop** (the `task-loop` recipe) when you want to *build* things — work through a plan, implement features, fix bugs
