 ...
 * EXPECTED OUTPUT:
 *   [FileManager] Started
 *   [FileManager] Analyzing 42 files in C:\Users\Brad\Downloads
 *   [Copilot] Suggested grouping strategy: by extension
 *   [DRY RUN] Would create: images\photo1.jpg
 *   [DRY RUN] Would create: documents\report.pdf
 *   [DRY RUN] Would create: archives\backup.zip
 ...
 *   await manager.start();
 *   await manager.organize({ targetFolder: "/path/to/folder", dryRun: true });
 *   await manager.stop();
 */
class FileOrganizer {
  ...
  private groupByExtension(files: FileInfo[], targetFolder: string): FileOperation[] {
    ...
    for (const file of files) {
      const subfolder = extensionMap[file.extension] || "other";
  ...
  private groupBySize(files: FileInfo[], targetFolder: string): FileOperation[] {
    ...
    for (const file of files) {
      ...
      if (file.size < 1024 * 1024) {
        subfolder = "small"; // < 1 MB
      } else if (file.size < 10 * 1024 * 1024) {
        subfolder = "medium"; // 1-10 MB
      } else {
        subfolder = "large"; // > 10 MB
      }
...
async function main() {
  ...
  try {
    ...
    // Example 1: Preview organization by extension (dry run)
    const downloadsFolder = path.join(process.env.HOME || process.env.USERPROFILE || "", "Downloads");

    console.log("\n=== Example 1: Organize by extension (dry run) ===");
    await organizer.organize({
      targetFolder: downloadsFolder,
      strategy: "extension",
      dryRun: true,
      excludePatterns: [".*", "*.tmp"], // Skip hidden files and temp files
    });

    // Example 2: Organize by date (apply changes)
    // Uncomment to run:
    // console.log("\n=== Example 2: Organize by date (apply) ===");
    // await organizer.organize({
    //   targetFolder: downloadsFolder,
    //   strategy: "date",
    //   dryRun: false,
