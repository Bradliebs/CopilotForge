...
const FULL_FILES = [
  ...
  // Oracle Prime agent (extras selection)
  {
    dest: path.join('.copilot', 'agents', 'oracle-prime.md'),
    content: templates.ORACLE_PRIME_AGENT_MD,
  },
...
async function run(args) {
  ...
  if (answersIdx !== -1 && args[answersIdx + 1]) {
    try {
      prefilledAnswers = JSON.parse(args[answersIdx + 1]);
    } catch {
  ...
  // Export prefilled answers for wizard consumption
  if (prefilledAnswers) {
    run._prefilledAnswers = prefilledAnswers;
  }

  // --oracle-prime: install only Oracle Prime files
  if (oraclePrimeOnly) {
    const oracleFiles = [
      {
        dest: path.join('.github', 'instructions', 'oracle-prime.instructions.md'),
        src: path.join('.github', 'instructions', 'oracle-prime.instructions.md'),
        type: 'copy',
      },
      {
        dest: path.join('.github', 'skills', 'oracle-prime', 'SKILL.md'),
        src: path.join('.github', 'skills', 'oracle-prime', 'SKILL.md'),
        type: 'copy',
      },
      {
        dest: path.join('.copilot', 'agents', 'oracle-prime.md'),
    ...
    if (dryRun) {
      for (const entry of oracleFiles) {
    ...
    banner();
    console.log(`  ${colors.bold('🔮 Installing Oracle Prime...')}`);
    console.log();
    ...
    console.log();
    if (created > 0) {
      success(`Oracle Prime installed! ${created} file(s) created.`);
      info('Reload VS Code to activate. Say "deep analysis" or "oracle prime" in Copilot Chat.');
    } else {
      info('Oracle Prime already installed. Use --yes to overwrite.');
    }
    console.log();
    return;
  }
  ...
  try {
    ...
    const allPaths = [...CORE_FILES, ...FULL_FILES.map((f) => f.dest)];
    captureSnapshot(allPaths, cwd);
  } catch { /* rollback is optional — don't block init */ }

  // Load project hooks and fire PreScaffold
  try {
    const { loadProjectHooks, fireHooks } = require('./hooks');
    loadProjectHooks(cwd);
    const hookResult = await fireHooks('PreScaffold', { cwd, minimal, beginner, files: CORE_FILES });
    if (hookResult.blocked) {
      warn('Scaffolding blocked by a PreScaffold hook.');
      console.log();
      return;
    }
