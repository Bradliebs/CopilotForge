...
async def main() -> None:
    ...
    config = ApiClientConfig(
        ...
        max_retries=2,
        auth_token=os.environ.get("API_TOKEN", "{{api_token}}"),
    )

    async with ApiClient(config) as client:
        try:
            # GET with query params.
            print("1. GET /{{resource_path}} (first 3):")
            resp = await client.get("/{{resource_path}}", params={"_limit": 3})
            for item in resp.data:
                print(f"   [{item['id']}] {item.get('name', item.get('title'))}")

            # POST request.
            print("\n2. POST /{{resource_path}}:")
            resp = await client.post(
                "/{{resource_path}}",
                json={"name": "New item from {{project_name}}"},
            )
            print(f"   Created #{resp.data['id']} (status: {resp.status_code})")
