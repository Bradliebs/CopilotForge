...
WHAT THIS DOES:
    ...
    ``mcp`` package. Registers example tools with type-hinted inputs,
...
async def data_lookup(collection: str, key: str) -> str:
    ...
    Args:
        collection: Which data collection to query.
    ...
    if collection not in store:
        return f"Error: Collection '{collection}' not found. Valid: {list(store.keys())}"
...
async def status_check(services: list[str]) -> str:
    ...
    for service in services:
        ...
        try:
            # TODO: Replace with actual health checks for {{project_name}}.
