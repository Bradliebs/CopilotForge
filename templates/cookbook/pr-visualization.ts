 ...
 *      npx ts-node cookbook/pr-visualization.ts --repo {{repo-owner}}/{{repo-name}}
 *
 * PREREQUISITES:
 *   - Node.js 18+
 *   - TypeScript 5+
 ...
 *
 * EXPECTED OUTPUT:
 *   [Detect] Repository: {{repo-owner}}/{{repo-name}}
...
// --- Mock PR Fetching ---
// TODO: {{pr-fetch-strategy}} — Replace with actual GitHub API calls or let
//       Copilot's built-in GitHub MCP Server tools handle PR fetching.

function fetchOpenPRs(repo: string): PullRequest[] {
  const now = Date.now();
  const day = 86_400_000;

  // TODO: Replace simulated data with real GitHub API integration.
