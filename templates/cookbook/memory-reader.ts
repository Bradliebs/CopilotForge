...
const DEFAULT_MEMORY_DIR = "{{memory_dir}}";
...
function formatContextBlock(summary: MemorySummary): string {
  ...
  const project = summary.decisions[0]?.title || "{{project_name}}";
  const stack = summary.conventions.stackConventions.slice(0, 3).join(", ") || "{{stack}}";
