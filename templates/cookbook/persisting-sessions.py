...
EXPECTED OUTPUT:
    ...
    [Assistant] [Echo] You said: "{{initial-prompt}}" ...
    [Session] Resumed: {{user-id}}-conversation (2 messages in history)
    [Assistant] [Echo] You said: "{{followup-prompt}}" ...
    [Sessions] 1 active session(s):
      - {{user-id}}-conversation (4 messages)
    [History] 4 message(s) in {{user-id}}-conversation
...
def main() -> None:
    ...
    try:
        ...
        session_id = build_session_id("{{user-id}}", "conversation")
        session = client.create_session(session_id)

        response1 = session.send("{{initial-prompt}}")
        ...
        response2 = resumed.send("{{followup-prompt}}")
