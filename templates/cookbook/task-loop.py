...
def parse_plan(file_path: Path) -> list[Task]:
    """
    Parse {{PLAN_FILE}} into a list of tasks.
...
def write_plan(file_path: Path, tasks: list[Task]) -> None:
    """Write the task list back to {{PLAN_FILE}}."""
...
def validate_task(task: Task) -> bool:
    ...
    TODO: Replace with actual validation (e.g., subprocess.run(["{{VALIDATE_COMMAND}}"])).
    """
...
def ralph_loop(plan_path: Path, max_iterations: int = {{MAX_ITERATIONS}}) -> None:
    """Main loop: pick task → implement → validate → commit → repeat."""
    if not plan_path.exists():
        print(f"[Ralph] Plan not found: {plan_path}")
        print("[Ralph] Create an {{PLAN_FILE}} with tasks like:")
